...
export function BuildingElementMesh({ 
  ...
  const renderCustomElement = () => {
    ...
    // Check if this is a union (geometric merge), intersection (geometric overlap), difference (geometric subtraction), or group (visual grouping)
    const isUnion = element.properties.unionOf;
    const isIntersection = element.properties.intersectionOf;
    const isDifference = element.properties.differenceOf;
    const isGroup = element.properties.groupOf;
    const hasCsgGeometry = element.properties.csgGeometry;

    if ((isUnion || isIntersection || isDifference) && hasCsgGeometry && element.properties.geometryData) {
    ...
    } else if (isUnion || isIntersection || isDifference) {
      // For simple unions/intersections/differences without CSG, render as bounding box
  ...
  return (
    ...
    >
      ...
      {element.type === 'custom' && (
        <>
          {/* If it's a CSG union/intersection/difference, render only the merged geometry (no bounding box) */}
          {(element.properties.unionOf || element.properties.intersectionOf || element.properties.differenceOf) && element.properties.csgGeometry && element.properties.geometryData && renderCustomElement()}
          
          {/* If it's a simple union/intersection/difference without CSG, render bounding box */}
          {(element.properties.unionOf || element.properties.intersectionOf || element.properties.differenceOf) && !element.properties.csgGeometry && (
            <>
              {getGeometry()}
              {getMaterial()}
            </>
      ...
      {(isSelected || isMultiSelected) && (
        <mesh>
          ...
          {element.type === 'custom' && (element.properties.unionOf || element.properties.intersectionOf || element.properties.differenceOf) && element.properties.csgGeometry && element.properties.geometryData ? (
            // For CSG unions/intersections/differences, create outline from CSG geometry
            (() => {
              const geometryData = element.properties.geometryData as GeometryData;
              const { positions, indices } = geometryData;
